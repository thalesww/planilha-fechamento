    ...
    async function start() {
      try {
        if (!window.isSecureContext) {
          throw new Error("A camera exige HTTPS ou localhost no celular.");
        }
        ...
        const scan = async () => {
          ...
          try {
            ...
            if (value) {
              ...
              setStatus("QR Code invalido para este fechamento. Aponte para o QR correto ou cole o conteudo abaixo.");
            }
          } catch {}
    ...
    <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Ler QR Code do fechamento">
      <section className="qr-modal">
        ...
        <button
          ...
          type="button"
          onClick={() => {
            const imported = onApply(manualText);
            if (!imported) setStatus("Conteudo colado invalido. Confira o texto copiado do computador.");
          }}
        >
          Importar conteúdo colado
        </button>
      </section>
    </div>
  );
